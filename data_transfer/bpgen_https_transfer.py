...
"""
Transfer data files from Blueprint Genetics cURLs to a dataset's GCP main-upload bucket.
"""
...
@click.option('--bpgen-curl-file-path')
def main(bpgen_curl_file_path: str):
    """
    ...
    with AnyPath(bpgen_curl_file_path).open() as file:
    ...
    for idx, curl in enumerate(owncloud_curls):
        url = curl.split(' ')[0]
        filename = os.path.basename(url).split('?Expires=')[0].removesuffix("'")
        if not filename:
